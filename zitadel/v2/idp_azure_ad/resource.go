...
func GetResource() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			idp_utils.TenantTypeVar: {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     "AZURE_AD_TENANT_TYPE_COMMON",
				Description: "the azure ad tenant type",
			},
			idp_utils.TenantIDVar: {
				Type:        schema.TypeString,
				Optional:    true,
				Description: fmt.Sprintf("if %s is not set, the %s is used", idp_utils.TenantIDVar, idp_utils.TenantTypeVar),
			},
			idp_utils.EmailVerifiedVar: {
