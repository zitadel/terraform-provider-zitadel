...
const (
	...
	SystemAPIKeyFileDesc      = "Path to the PEM encoded private key for a ZITADEL System API user. Either 'key_file' or 'key' is required when using System API authentication."
	SystemAPIKeyAttr          = "key"
	SystemAPIKeyDesc          = "PEM encoded private key for a ZITADEL System API user. Either 'key_file' or 'key' is required when using System API authentication."
...
func GetClientInfo(ctx context.Context, insecure bool, domain string, accessToken string, token string, jwtFile string, jwtProfileFile string, jwtProfileJSON string, systemAPIKeyFile string, systemAPIKey string, systemAPIUser string, systemAPIAudience string, port string, insecureSkipVerifyTLS bool, transportHeaders map[string]string) (*ClientInfo, error) {
	...
	if accessToken != "" {
	...
	} else if systemAPIKeyFile != "" || systemAPIKey != "" {
		...
		return nil, fmt.Errorf("either 'access_token', 'jwt_file', 'jwt_profile_file', 'jwt_profile_json' or 'system_api' (with 'key' or 'key_file') is required")
	}
