...
subcategory: ""
description: |-
  Customizes the text displayed in the legacy login UI (v1) at /ui/login. These settings have no effect on the new hosted login v2 (/ui/v2/login). Hosted login v2 translations are managed by zitadel_hosted_login_translation. Instance-level defaults are managed by zitadel_default_login_texts.
---
